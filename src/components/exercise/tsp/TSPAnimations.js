...
const ResponsiveNetworkAnimation = ({
  solution,
  weights: distances,
  test = false,
}) => {
...
const DebugOptions = ({ nodes, links }) => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      for (let i = 0; i < solution.length - 1; i++) {
        ...
        const city = solution[i];
        const nextCity = solution[i + 1];
        const distance = weights[city][nextCity];
  ...
  return (
    <>
      ...
      <h1>Reset Links</h1>
      <Button onClick={() => links.reset()}>Reset</Button>
      <h1>Reset Nodes</h1>
      <Button onClick={() => nodes.reset()}>Reset</Button>
      <h1>Stop</h1>
      <Button
        onClick={async () => {
          links.reset();
